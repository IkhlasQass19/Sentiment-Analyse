...
class InputApplication(tk.Tk):
    ...
    def select_image(self):
        ...
        if image_path:
            ...
            image_window.title("Sentiment of selected Image")
            ...
            label = tk.Label(image_frame, image=img, bg="#ffffff")
            label.image = img
            label.grid(row=0, column=0)

            # Prédiction du sentiment
            input_data = np.array(resize_image) / 255
            pred = loaded_image_model.predict(np.expand_dims(input_data, axis=0))
            result = pred.argmax()
            sentiment = "Positive 😄" if result == 3 else "Negative 😔"

            # Ajouter une étiquette de texte pour afficher le sentiment détecté
            sentiment_label = tk.Label(image_frame, text=f"Sentiment: {sentiment}", bg="#ffffff")
            sentiment_label.grid(row=1, column=0, pady=(20, 0))
